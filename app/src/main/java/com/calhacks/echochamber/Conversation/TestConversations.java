...
public class TestConversations {
    ...
    public static Conversation[] getConversations() {
        ...
        conversations[0] = new Conversation(topics[0], "ignore1");
        conversations[0].addMessage(new Message(new Date(), true, "Where you at?"));
        conversations[0].setPartnerName("Max G.");
        conversations[0].setActive(false);

        conversations[1] = new Conversation(topics[1], "ignore2");
        Date date1 = new Date();
        date1.setTime(date1.getTime() - 100000);
        conversations[1].addMessage(new Message(date1, false, "I'm dreaming right now."));
        conversations[1].setPartnerName("Connor B.");
        conversations[1].setActive(false);

        Date date2 = new Date();
        date2.setTime(date2.getTime() - 172800000);
        conversations[2] = new Conversation(topics[2], "ignore3");
        conversations[2].setPartnerName("Tony O.");
